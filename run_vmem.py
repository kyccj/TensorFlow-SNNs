"""문턱 근접도로 가중 — 침묵 뉴런을 갈라본다 (26-08-25 등록)

문제 — spike_count 는 정수 0..T 이고 T=4 다. 그래서 1-maxnorm 의 sc_rate = 1 - sc/max 는
많아야 5단계 계단이고, **한 번도 안 터진 뉴런은 전부 정확히 1.0** 이다.
그런데 실측상 안 터지는 뉴런이 층마다 78~97% 다 (reg_neuron_detail.csv, 채널 내 α=0.5):

    n_conv1 80.8%   n_conv3 83.5%   n_conv4_1 93.8%   n_conv5_1 78.2%   n_fc2 96.6%

wta_rev 의 backward 는 sc_rate/||x|| 라 **안 터진 뉴런에도 gradient 가 간다**.
즉 gradient 의 78~97% 가 전부 같은 값이다. 가중치를 아무리 정교하게 설계해도
나머지 3~22% 에만 적용되니 사실상 균일 가중(plain L2)이 된다.
**여태 모든 가중치 변형(softmax / maxnorm / 균일 / p-norm / entropy)이 한 곡선에
겹친 이유가 이것일 수 있다.**

방법 — 뉴런이 아직 들고 있는 전하를 문턱 대비 비율로 더해 연속값으로 만든다.

    sc_soft = spike_count + gain * clip(vmem/vth, 0, 1)

vmem 은 이 시각의 **발화 후** 막전위라, 방금 터진 뉴런은 낮다 (그건 spike_count 가 이미
반영한다). 침묵 뉴런만 "거의 터질 뻔했다" vs "한참 미달" 로 갈린다.
gain=0 이면 현재 동작 그대로다.

주의 — gain 을 올리면 mean(sc_rate) 가 내려간다 (~0.96 -> ~0.83 @ gain=4). 실효 lambda 가
그만큼(최대 14%) 약해진다. lambda 간격이 2배씩이라 이보다 작고, 어차피
**실제 스파이크 수 x 실제 정확도 평면에서 곡선끼리** 비교하므로 보정하지 않는다.

- flags.py : reg_spike_vmem_gain (float, 기본 0.0)
- lib_snn/neurons.py : 1-maxnorm 분기에서 scp 를 sc_soft 로 대체

대조군 (gain=0, 같은 방법 = 채널 내 1-maxnorm, V16-C10):
    lambda=1e-7  94.72% @ 49,963
    lambda=3e-7  94.46% @ 29,646
    lambda=5e-7  94.53% @ 20,710   <- 이번 실험과 같은 lambda
    규제 없음     95.00% @ 78,189

판정 — gain 을 올렸을 때 같은 스파이크에서 정확도가 올라가면 "침묵 뉴런이 구분되지 않는 것"이
가중치 라인의 병목이었다. 곡선이 겹치면 병목은 다른 데 있다.
반복 편차 sd 0.17~0.28%p, 전부 n=1 이므로 0.3%p 미만 차이는 판정 불가.
"""
import subprocess, os, threading, time, sys, re

PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_vmem')
...
#        이름        gain    lambda
JOBS = [
    ('g2_5e-7', '2.0', '5E-7'),   # 먼저 (중간 세기)
    ('g4_5e-7', '4.0', '5E-7'),   # 가장 세게 가름
    ('g1_5e-7', '1.0', '5E-7'),   # 자연스러운 세기 — GPU 0 비면
]
...
def make_config(gpu_id, name, gain, lmb):
    ...
    c, n = RE_NAME.subn(f"conf.exp_set_name='vm-{name}'", c)
    if n < 1:
    ...
    c, n = RE_MXG.subn(lambda m: (f"{m.group(1)}conf.reg_spike_maxnorm_group = 'within_channel'\n"
                                  f"{m.group(1)}conf.reg_spike_vmem_gain = {gain}"), c, count=1)
    if n != 1:
...
def verify(path, name, gpu, gain, lmb):
    ...
    for bad, why in [(r"^\s*conf\.reg_spike_out_sc_maxnorm_plain\s*=\s*True\b", 'maxnorm_plain'),
                     ...
                     (r"^\s*conf\.reg_spike_wta_rev_floor\s*=", 'floor (이번 실험과 섞이면 안 됨)')]:
...
def preflight(jobs):
    ...
    for name, gain, lmb in jobs:
        tgt = os.path.join(PROJECT_ROOT, f'vm-{name}')
    ...
    if not re.search(r"reg_spike_vmem_gain", open(os.path.join(PROJECT_ROOT, 'flags.py')).read()):
        bad.append('flags.py 에 reg_spike_vmem_gain 이 없음')
    if not re.search(r"reg_spike_vmem_gain",
                     open(os.path.join(PROJECT_ROOT, 'lib_snn', 'neurons.py')).read()):
        bad.append('lib_snn/neurons.py 에 vmem gain 반영 안 됨')
    if bad:
        raise SystemExit('중단:\n  ' + '\n  '.join(bad))
    print('사전 점검 통과: 저장 경로 충돌 없음, gain 플래그 반영됨', flush=True)
...
def main():
    ...
    preflight(pending)
    print(f'--- vmem readiness sweep: {len(pending)} runs ---', flush=True)
    while pending:
