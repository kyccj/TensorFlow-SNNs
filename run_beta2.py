"""승자 장려 — beta > 1 (26-08-27 등록)

    sc_rate = 1 - beta * (sc/max)

beta>1 이면 많이 터진 뉴런의 sc_rate 가 **음수**가 되어 gradient 부호가 뒤집힌다.
지금까지는 승자에게 sc_rate=0 을 줘서 **안 누를 뿐**이었는데, 이건 **더 터지게 민다.**
WTA 의 나머지 절반("승자는 더 발화")을 처음으로 구현하는 것이다.

gradient (lambda=5e-7, 실측 분포 기준):
    sc   beta=1        beta=1.5      beta=2
    0    +2.13e-07     +2.18e-07     +1.21e-07     (침묵 - 누름)
    3    +5.33e-08     -2.73e-08     -6.06e-08     (음수 = 밀어줌)
    4     0            -1.09e-07     -1.21e-07

왜 하나 — 14가지 방법을 재봤는데 **살아있는 뉴런의 발화수가 2.34~2.56 에 갇혀 있다**
(T=4 가 상한인데 60% 를 안 쓴다). 침묵 비율은 85~93% 로 크게 움직이는데 발화 강도는 안 변한다.
경쟁을 시킨 몫도 +1.6% 뿐이다 (채널간 flat 2.373 vs 채널간 α=7 2.401).
즉 지금 규제는 **뉴런을 끄기만 하고 남은 것의 강도는 안 건드린다** — 가지치기이지 WTA 가 아니다.

밀어주는 힘이 아예 없었기 때문일 수 있다. beta>1 이 그 힘을 넣는다.

판정 — **정확도가 아니라 기전으로 본다.**
    침묵 비율이 비슷한데 살아있는 것의 발화수가 2.4 -> 3.0 이상으로 오르면
    처음으로 "winner takes more" 가 나타난 것이다.
    (지금까지 어떤 방법도 이 값을 못 움직였다.)
정확도가 나빠져도 기전 관찰로서 값어치가 있다.

대조군 — 같은 lambda=5e-7, 채널 내 1-maxnorm:
    beta=1.0  94.53% @ 20,710  (침묵 93.45%, 살아있는것 발화수 2.477)
    beta 0~1  스윕이 방금 끝남 (_beta/)
mean(sc_rate) = 1 - beta*0.04 이라 beta=2 에서도 0.92 로 양수를 유지한다.
"""
import subprocess, os, threading, time, sys, re

PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_beta2')
...
#        이름        gain    lambda
JOBS = [
    ('b150', '1.5', '5E-7'),   # 승자에게 음수 gradient (밀어줌)
    ('b200', '2.0', '5E-7'),   # 더 세게
]
...
def make_config(gpu_id, name, gain, lmb):
    ...
    c, n = RE_NAME.subn(f"conf.exp_set_name='b2-{name}'", c)
    if n < 1:
...
def preflight(jobs):
    ...
    for name, gain, lmb in jobs:
        tgt = os.path.join(PROJECT_ROOT, f'b2-{name}')
...
def main():
    ...
    preflight(pending)
    print(f'--- beta>1 winner-push: {len(pending)} runs ---', flush=True)
    while pending:
