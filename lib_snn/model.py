    ...
    #def __init__(self, batch_size, input_shape, data_format, num_class, conf, **kwargs):
        ...
        if conf.en_stdp_pathway or conf.spike_trace_debug:
            if conf.model=='VGG16':
                ...
                n_conv2_block3_conv1 = self.get_layer('conv2_block3_conv1_n')
                n_conv2_block3_out = self.get_layer('conv2_block3_out_n')
                n_conv2_block4_conv1 = self.get_layer('conv2_block4_conv1_n')
                n_conv2_block4_out = self.get_layer('conv2_block4_out_n')
                n_conv2_block5_conv1 = self.get_layer('conv2_block5_conv1_n')
                n_conv2_block5_out = self.get_layer('conv2_block5_out_n')
                ...
                n_conv3_block3_conv1 = self.get_layer('conv3_block3_conv1_n')
                n_conv3_block3_out = self.get_layer('conv3_block3_out_n')
                n_conv3_block4_conv1 = self.get_layer('conv3_block4_conv1_n')
                n_conv3_block4_out = self.get_layer('conv3_block4_out_n')
                n_conv3_block5_conv1 = self.get_layer('conv3_block5_conv1_n')
                n_conv3_block5_out = self.get_layer('conv3_block5_out_n')
                ...
                n_conv4_block3_conv1 = self.get_layer('conv4_block3_conv1_n')
                n_conv4_block3_out = self.get_layer('conv4_block3_out_n')
                n_conv4_block4_conv1 = self.get_layer('conv4_block4_conv1_n')
                n_conv4_block4_out = self.get_layer('conv4_block4_out_n')
                n_conv4_block5_conv1 = self.get_layer('conv4_block5_conv1_n')
                n_conv4_block5_out = self.get_layer('conv4_block5_out_n')
                ...
                self.get_layer('conv2_block4_conv1').set_n_pre_post(n_conv2_block3_out,n_conv2_block4_conv1)
                self.get_layer('conv2_block4_conv2').set_n_pre_post(n_conv2_block4_conv1,n_conv2_block4_out)

                self.get_layer('conv2_block5_conv1').set_n_pre_post(n_conv2_block4_out,n_conv2_block5_conv1)
                self.get_layer('conv2_block5_conv2').set_n_pre_post(n_conv2_block5_conv1,n_conv2_block5_out)
                ...
                self.get_layer('conv3_block4_conv1').set_n_pre_post(n_conv3_block3_out,n_conv3_block4_conv1)
                self.get_layer('conv3_block4_conv2').set_n_pre_post(n_conv3_block4_conv1,n_conv3_block4_out)

                self.get_layer('conv3_block5_conv1').set_n_pre_post(n_conv3_block4_out,n_conv3_block5_conv1)
                self.get_layer('conv3_block5_conv2').set_n_pre_post(n_conv3_block5_conv1,n_conv3_block5_out)
                ...
                self.get_layer('conv4_block4_conv1').set_n_pre_post(n_conv4_block3_out,n_conv4_block4_conv1)
                self.get_layer('conv4_block4_conv2').set_n_pre_post(n_conv4_block4_conv1,n_conv4_block4_out)

                self.get_layer('conv4_block5_conv1').set_n_pre_post(n_conv4_block4_out,n_conv4_block5_conv1)
                self.get_layer('conv4_block5_conv2').set_n_pre_post(n_conv4_block5_conv1,n_conv4_block5_out)
...
#            self.spike_max_pool_setup()
    ...
    def spike_max_pool_setup(self):
        ...
        for depth in depth_keys:
            ...
            for node in nodes:
                ...
                _node_layer = getattr(node, 'layer', None) or getattr(node, 'operation', None)
                if isinstance(_node_layer, lib_snn.layers.MaxPool2D):
                    _node_layer.prev_layer = prev_layer
                    _node_layer.prev_layer_set_done = True

                prev_layer = _node_layer
    ...
    def _run_internal_graph(self, inputs, training=None, mask=None):
        """Keras 3 compatibility shim: _run_internal_graph → _run_through_graph."""
        if not hasattr(self, '_run_through_graph'):
            # Keras 2 path
            return super()._run_internal_graph(inputs, training=training, mask=mask)

        def _op_fn(op):
            def _call(*args, **kwargs):
                if training is not None:
                    try:
                        return op(*args, training=training, **kwargs)
                    except TypeError:
                        return op(*args, **kwargs)
                return op(*args, **kwargs)
            return _call

        result = self._run_through_graph(inputs, operation_fn=_op_fn)
        return result

    def _validate_target_and_loss(self, y, loss):
        """Keras 3 compatibility shim — validation now handled by Keras internally."""
        pass
    ...
    def train_step_snn_gradient(self, data):
        ...
        if f_grad_accum:
            ...
            if False:
                ...
                result = self.compute_metrics(x, y, y_pred, sample_weight)
                result['loss'] = loss
                return result
    ...
    def init_snn(self, model_ann=None):
        ...
        # Keras 3: compiled_metrics is DeprecatedCompiledMetric with no _metrics
        # Fall back to _compile_metrics._user_metrics if available
        if hasattr(self.compiled_metrics, '_metrics'):
            metrics = self.compiled_metrics._metrics
        elif hasattr(self, '_compile_metrics') and hasattr(self._compile_metrics, '_user_metrics'):
            metrics = self._compile_metrics._user_metrics or []
        else:
            metrics = []
    ...
    def set_layers_w_neuron(self):
        ...
        def _check_and_add(layer):
            if id(layer) in seen:
                return
            seen.add(id(layer))
            # Keras 3: model.layers exposes Neuron sub-layers directly
            if isinstance(layer, lib_snn.neurons.Neuron):
                self.layers_w_neuron.append(layer)
            elif isinstance(layer, lib_snn.activations.Activation):
                if isinstance(layer.act, lib_snn.neurons.Neuron):
                    self.layers_w_neuron.append(layer)

        for layer in self.layers:
            _check_and_add(layer)
    ...
    def _run_internal_graph_snn_t_first(self, inputs, training=None, mask=None):
        """Dispatch to Keras 2 or Keras 3 version."""
        if hasattr(self, '_tensor_usage_count'):
            # Keras 2
            return self._run_internal_graph_snn_t_first_k2(inputs, training=training, mask=mask)
        else:
            # Keras 3
            return self._run_internal_graph_snn_t_first_k3(inputs, training=training, mask=mask)

    def _run_internal_graph_snn_t_first_k2(self, inputs, training=None, mask=None):
        """Run internal graph - SNN, temporal first (Keras 2)."""
            ...
            for node in nodes:
                ...
                if node.is_input:
                    ...
                    if conf.input_data_time_dim:
                        _input_one_ts_shape = _input.shape[0:1].concatenate(_input.shape[2:])
                    else:
                    ...
                    layers_temporal_reduction = [lib_snn.layers.BatchNormalization]
    ...
    def _run_internal_graph_snn_t_first_k3(self, inputs, training=None, mask=None):
        """Run internal graph - SNN, temporal first (Keras 3)."""
        ...
        for depth in depth_keys:
            ...
            for node in nodes:
                ...
                layer_name = layer.name if layer is not None else 'input'
                tensor_array_name = 'out_arr' + layer_name

                if node.is_input:
                    x = node.output_tensors[0]
                    _input = tensor_dict[id(x)]
                    ...
                    tensor_dict[id(x)] = layer_out

                else:
                    if any(id(t) not in tensor_dict for t in node.input_tensors):
                        assert False, 'Node inputs not available: ' + layer_name
                        continue
                    ...
                    if hasattr(layer, 'temporal_mean_input') and layer.temporal_mean_input:
                        f_temporal_reduction = True

                    f_temporal_batch = False
                    if hasattr(layer, 'temporal_batch') and layer.temporal_batch:
                        f_temporal_batch = True
                    ...
                    if f_temporal_batch:
                        ...
                        for t in range(1, self.conf.time_step + 1):
                            if isinstance(layer_in, list):
                                _layer_in = [li.read(t - 1) for li in layer_in]
                            else:
                                _layer_in = layer_in.read(t - 1)
                    ...
                    for x, y in zip(node.outputs, tf.nest.flatten(layer_out)):
                        tensor_dict[id(x)] = y

        output_tensors = []
        for x in self.outputs:
            assert id(x) in tensor_dict, 'Could not compute output ' + str(x)
            _output_tensors = tensor_dict[id(x)].stack()
