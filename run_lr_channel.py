...
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_lrch')
...
JOBS = [
    ('v16c10_ch',   True,  False),
    ('v16c10_glob', False, False),
    ('r19c10_ch',   True,  True),
    ('r19c10_glob', False, True),
]
...
def make_config(gpu, name, chan, r19):
    ...
    sub(RE_NAME, f"conf.exp_set_name='lrch-{name}'",            'exp_set_name')
    ...
    if chan:                       # α 줄 바로 뒤에 채널 간 묶음을 켠다
        sub(RE_ALPHA, lambda m: m.group(0) + f"\n{m.group(1)}conf.reg_spike_sm_group = 'channel'",
            'sm_group')
    if r19:
        sub(RE_MODEL, "conf.model='ResNet19'", 'model')
        sub(RE_DATA,  "conf.dataset='CIFAR10'", 'dataset')
    return c
...
def verify(path, name, gpu, chan, r19):
    ...
    want = 'ResNet19' if r19 else 'VGG16'
    if not re.search(rf"^conf\.model='{want}'$", a, re.M):
        raise RuntimeError(f'{name}: model 이 {want} 아님')
...
def preflight(jobs):
    ...
    for name, _c, _r in jobs:
        t = os.path.join(PROJECT_ROOT, f'lrch-{name}')
...
def run_one(gpu, name, chan, r19):
    ...
    print(f"[{time.strftime('%m-%d %H:%M')}] [GPU {gpu}] START {name} "
          f"(채널가중치={chan}, R19={r19}, ρ={RHO})", flush=True)
