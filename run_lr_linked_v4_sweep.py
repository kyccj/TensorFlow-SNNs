"""
LR-linked lambda v4: fine-tuning sweet spot near baseline accuracy.
Target: >=94.85% accuracy with maximum spike reduction.
  - hybrid+p2 with lower lambda (2e-7, 2.5e-7)
  - higher power (p=4) to push reg further to end
  - hybrid+p3 with lower lambda
VGG-C10, 310 epochs
"""
...
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_lr_linked_v4')
PYTHON = '/home/kyccj/anaconda3/envs/venv_1/bin/python'
CUDA_LD_PATH = '/home/kyccj/anaconda3/envs/venv_1/lib'

# (name, gpu, lmax, power, start_ep, safety)
EXPERIMENTS = [
    ('hybrid_p2_lmax2e-7',   0, 2e-7, 2.0, 20, True),
    ('hybrid_p2_lmax2.5e-7', 1, 2.5e-7, 2.0, 20, True),
    ('hybrid_p3_lmax2e-7',   2, 2e-7, 3.0, 20, True),
    ('hybrid_p3_lmax3e-7',   3, 3e-7, 3.0, 20, True),
    ('power4_lmax3e-7',      4, 3e-7, 4.0, 20, False),
    ('power4_lmax5e-7',      5, 5e-7, 4.0, 20, False),
]
...
def generate_config(name, gpu_id, lmax, power, start_ep, safety, run_dir):
    ...
    content = content.replace(
        ...
        f"conf.exp_set_name='lr-linked-v4-{name}'"
    )
