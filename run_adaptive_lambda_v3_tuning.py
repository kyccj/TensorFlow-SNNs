'''
    Adaptive Lambda v2 tuning: find better acc-spike tradeoff

    Problem: v2 su1.0 pushed λ to max(1e-5) → acc -1.1%
    Goal: maintain acc within -0.5% while maximizing spike reduction

    Strategies:
    - Lower lambda_max to cap overshoot
    - Larger step_down to recover faster from accuracy drops
    - Wider margin to tolerate more accuracy fluctuation

    GPU 2: VGG-C10, su1.0, sd0.5, lambda_max=1e-6 (cap λ earlier)
    GPU 3: VGG-C10, su1.0, sd0.3, lambda_max=5e-7 (even tighter cap)
    GPU 4: VGG-C10, su0.5, sd0.3, lambda_max=1e-5 (slower growth)
    GPU 5: VGG-C10, su1.0, sd0.5, margin=0.003 (tighter accuracy gate)
'''
import subprocess
import os
...
def generate_config(exp):
    ...
    content = content.replace(
        "#\nconfig.set()",
        f"# adaptive lambda v2 tuning: {exp['label']}\n"
    ...
    content = content.replace(
        ...
        f"conf.exp_set_name='adp-v2t-{model_short}-{ds_short}-{label}'"
    )
...
def main():
    ...
    for exp in EXPERIMENTS:
        ...
        print(f'[GPU {exp["gpu"]}] VGG-C10 {exp["label"]} '
    ...
    try:
        for exp, p, log_file in processes:
            ...
            print(f'[GPU {exp["gpu"]}] {exp["label"]} finished: {status}')
