"""1-s 곡선 무손실 경계 채우기 — lambda 1.5e-7 / 2e-7 (26-08-29 등록)

전역 1-softmax 곡선(모든 비교의 기준선)이 이렇게 비어 있다:
    53,160 @ 94.89% (lambda=1e-7, n=3)  ->  [빈 구간]  ->  33,175 @ 94.34% (lambda=3e-7)

무손실 기준은 95.00 - 2sd = **94.78%** 이고, 경계가 저 빈 구간 안에 있다.
지금은 "무손실 최심점 = 53,160" 이라고 말하고 있지만 **실제로는 더 깊을 수 있다.**

이게 중요한 이유 — 다른 방법의 이득을 이 경계와 견줘서 말하기 때문이다.
    vmem silent_only  94.59% @ 36,567 (n=3)   무손실 X
    vmem lambda 3e-7 / 3.5e-7 이 ~46,600 / ~43,300 에 착지 예정
경계가 53,160 이면 vmem 이 46,600 에서 무손실일 때 12% 개선이지만,
경계가 45,000 이면 개선이 없다. **기준이 정해져야 판정이 된다.**

전례 — 어제 20,244 에 점 하나를 찍었더니 직선 보간이 0.29%p 낮게 잡고 있던 것이 드러나,
가장 유망하던 후보가 +0.48%p -> +0.18%p 로 깎였다. 빈 구간은 반드시 채워야 한다.

lambda 는 실측 외삽 (스파이크 ~ lambda^-0.561):
    lambda=1.5e-7 -> ~42,600     lambda=2e-7 -> ~36,300

설정 — `_sweep_wta_rev` 와 동일한 순수 기준선.
maxnorm / 채널 묶음 / vmem / accum / beta 전부 끔, 전역 1-softmax + wta_rev 만.
"""
import subprocess, os, threading, time, sys, re

PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_gap3')
...
#        이름        gain    lambda
JOBS = [
    ('sm_1.5e-7', '1.0', '1.5E-7'),   # 예상 ~42,600
    ('sm_2e-7',   '1.0', '2E-7'),     # 예상 ~36,300
]
...
def make_config(gpu_id, name, gain, lmb):
    ...
    c, n = RE_NAME.subn(f"conf.exp_set_name='g3-{name}'", c)
    if n < 1:
...
def preflight(jobs):
    ...
    for name, gain, lmb in jobs:
        tgt = os.path.join(PROJECT_ROOT, f'g3-{name}')
...
def main():
    ...
    preflight(pending)
    print(f'--- 기준선 무손실 경계: {len(pending)} runs ---', flush=True)
    while pending:
