"""gain=2 의 λ 스윕 — 곡선인가 점 하나인가 (26-08-25 등록)

배경 — vmem gain=2, λ=5e-7 이 95.05% @ 37,139 을 냈다.
규제 없음이 95.00% @ 78,189 이므로 **정확도 유지 + 스파이크 52.5% 감소**다.
지금까지 확실히 무손실이던 가장 깊은 점(~53,000)보다 29% 더 깊다.

문제 — 점이 하나다. 두 가지를 못 가른다:
  (a) gain=2 가 곡선 전체를 올렸다
  (b) 37,139 이라는 착지점이 우연히 좋았다
게다가 gain 을 올리면 mean(sc_rate) 가 내려가 실효 λ 가 약해지므로,
같은 λ=5e-7 이라도 gain=0(20,710) 과 gain=2(37,139) 는 **같은 세기가 아니다**.
곡선을 그려 같은 스파이크에서 비교해야 한다.

대조군 — 채널 내 1-maxnorm, gain=0 (같은 방법, gain 만 0):
    λ=1e-7  94.72% @ 49,963
    λ=3e-7  94.46% @ 29,646
    λ=5e-7  94.53% @ 20,710
37,139 에서의 보간값은 94.56% 이므로, gain=2 의 그 점은 +0.49%p 에 해당한다.

규제 없음 95.00% (sd 0.11, n=5) / 78,189. 무손실 기준 = 94.78% (baseline − 2sd).

이번에 돌리는 것 — gain=2 고정, λ 5점 (5e-7 은 반복이라 n=2 가 된다).
판정 — gain=2 곡선이 gain=0 곡선보다 위에 있으면 (a), 5e-7 만 튀어있으면 (b).
반복 편차 sd 0.17~0.28%p 이므로 0.3%p 미만 차이는 판정 불가.
"""
import subprocess, os, threading, time, sys, re

PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_vmlam')
...
#        이름        gain    lambda
JOBS = [
    ('g2_2e-6', '2.0', '2E-6'),   # ~20K 예상 — gain=0 의 5e-7 자리와 맞물릴 것
    ('g2_1e-6', '2.0', '1E-6'),   # ~28K 예상
    ('g2_4e-6', '2.0', '4E-6'),   # ~15K 예상, 가장 깊은 점
    ('g2_2e-7', '2.0', '2E-7'),   # ~50K 예상, 무손실 구간 안쪽
    ('g2_5e-7_r2', '2.0', '5E-7'),# 95.05% @ 37,139 의 반복 (n=2)
]
...
def make_config(gpu_id, name, gain, lmb):
    ...
    c, n = RE_NAME.subn(f"conf.exp_set_name='vml-{name}'", c)
    if n < 1:
...
def preflight(jobs):
    ...
    for name, gain, lmb in jobs:
        tgt = os.path.join(PROJECT_ROOT, f'vml-{name}')
...
def main():
    ...
    preflight(pending)
    print(f'--- vmem gain=2 lambda sweep: {len(pending)} runs ---', flush=True)
    while pending:
