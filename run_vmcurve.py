"""vmem silent_only 곡선 스윕 (26-08-30 등록)
...
**3e-7 의 95.08% 가 유일한 무손실 점이고 n=1 이다.** 바로 옆 3.5e-7 이 94.62% 로
0.46%p 낮은데 스파이크는 9% 차이뿐이라, 곡선이 아니라 편차로 보인다.
이 프로젝트에서 n=1 큰 값은 네 번 다 내려앉았다
(95.05->94.77, 94.53->94.19, 94.74->94.59, +0.78->+0.18).

점 하나를 반복하는 대신 **곡선을 그린다.** 곡선이 일관되게 기준선 위면
한 점의 n 을 늘리는 것보다 훨씬 강한 근거가 된다.

곡선 지수 0.481 (46,749 @ 3e-7, 36,567 @ 5e-7) 로 외삽:
    lambda=2e-7 -> ~56,800     lambda=4e-7 -> ~40,700

이번 4 run:
    3e-7 x2  (기존과 합쳐 n=3)   <- 핵심 점 확정
    2e-7 x1, 4e-7 x1            <- 곡선 양쪽

**같이 돌리는 것** — 1-s lambda=1.26e-7 x2. vmem 의 46,749 자리에 기준선 점이 없어서
보간(94.79%)에 의존하고 있는데, 그게 딱 무손실 경계라 판정이 안 된다.
1-s 도 거기서 무손실이면 vmem 의 이득은 0 이다.

기준선 현황 — 53,160 @ 94.89% (n=3, 무손실 O)  ->  [빈 구간]  ->  40,832 @ 94.66% (X)
규제 없음 95.00% (sd 0.11, n=5) @ 78,789.  무손실 기준 94.78%.
"""
import subprocess, os, threading, time, sys, re

PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_vmcurve')
...
#        이름        gain    lambda
JOBS = [
    ('g100_3e-7_r2', '1.0', '3E-7'),     # 핵심 점 반복 (기존과 합쳐 n=3)
    ('g100_3e-7_r3', '1.0', '3E-7'),
    ('g100_2e-7',    '1.0', '2E-7'),     # 예상 ~56,800
    ('g100_4e-7',    '1.0', '4E-7'),     # 예상 ~40,700
]
...
def main():
    ...
    preflight(pending)
    print(f'--- vmem silent_only 곡선: {len(pending)} runs ---', flush=True)
    while pending:
