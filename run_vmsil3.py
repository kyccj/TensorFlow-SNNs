"""vmem silent_only 를 무손실 구간으로 — lambda 3e-7 / 3.5e-7 (26-08-29 등록)

lambda=5e-7 에서 n=3 결과가 나왔다:  **94.59% @ 36,567  (sd 0.19, 흩어짐 1.0%)**
    개별 94.74 / 94.66 / 94.38
무손실 기준(95.00 - 2sd = 94.78%)에서 **-0.19%p**. 경계 아래다.
(첫 run 94.74% 가 셋 중 최고값이었다 — 이 프로젝트에서 네 번째로 같은 일이 반복됐다.)

**lambda 를 줄여 무손실 구간으로 올린다.** 스파이크 ~ lambda^-0.475 (1-m 곡선 지수) 외삽:
    lambda=3.0e-7  ->  ~46,600
    lambda=3.5e-7  ->  ~43,300

**판정** — 목표는 1-s 의 무손실 최심점보다 적은 스파이크에서 94.78% 이상을 내는 것이다.
    1-s lambda=1e-7  **94.89% @ 53,160  (n=3, 94.90/94.91/94.86)**  <- 현재 무손실 최심점
    46,600 에서 94.78% 이상이면 경계를 12% 민다.
    43,300 이면 18%.
못 넘으면 vmem 축은 닫는다.

**주의** — 1-s 곡선도 53,160 과 33,175 사이가 비어 있어서, 진짜 무손실 경계가
53,160 보다 깊을 수 있다. 그러면 vmem 의 이득이 그만큼 줄어든다. 그 구간을 채우는
실험(1-s lambda=1.5e-7, 2e-7)도 같이 필요하다.
...
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_vmsil3')
...
#        이름        gain    lambda
JOBS = [
    ('g100_3e-7',   '1.0', '3E-7'),     # 예상 ~46,600
    ('g100_3.5e-7', '1.0', '3.5E-7'),   # 예상 ~43,300
]
...
def make_config(gpu_id, name, gain, lmb):
    ...
    c, n = RE_NAME.subn(f"conf.exp_set_name='v3-{name}'", c)
    if n < 1:
...
def preflight(jobs):
    ...
    for name, gain, lmb in jobs:
        tgt = os.path.join(PROJECT_ROOT, f'v3-{name}')
...
def main():
    ...
    preflight(pending)
    print(f'--- vmem silent_only 무손실 구간: {len(pending)} runs ---', flush=True)
    while pending:
