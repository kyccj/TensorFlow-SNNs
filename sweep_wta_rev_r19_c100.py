'''
    WTA revised sweep - ResNet19 CIFAR100
    - wta_rev=True (reduce_mean instead of L2 norm)
    - alpha=7 (fixed, best from alpha sweep)
    - reg_spike_out_const: [1e-10, 1e-9, 5e-8, 1e-7, 5e-7, 1e-6, 5e-6, 1e-5]
    - ResNet19 + CIFAR100, 310 epochs
    - GPU 0~7
'''
...
SWEEP_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '_sweep_wta_rev_r19_c100')
LAMBDA_VALUES = [1e-10, 1e-9, 5e-8, 1e-7, 5e-7, 1e-6, 5e-6, 1e-5]
...
def generate_config(lmb, gpu_id, run_dir):
    ...
    # Keep ResNet19 + CIFAR100 (no dataset change needed - config default is CIFAR100)

    # exp name
    content = content.replace(
        "conf.exp_set_name='EIP-SNN-26'",
        f"conf.exp_set_name='EIP-SNN-26_wta-rev-r19-c100-lambda-{lmb}'"
    )
