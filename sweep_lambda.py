'''
    Lambda (reg_spike_out_const) sweep script
    - alpha=7 (fixed, best from alpha sweep)
    - reg_spike_out_const: [1e-10, 1e-9, 5e-7, 1e-6, 5e-6, 1e-5]
...
SWEEP_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '_sweep_lambda')
LAMBDA_VALUES = [1e-10, 1e-9, 5e-7, 1e-6, 5e-6, 1e-5]
...
def generate_config(lmb, gpu_id, run_dir):
    ...
    content = content.replace(
        ...
        f"conf.exp_set_name='EIP-SNN-26_sweep-lambda-{lmb}'"
    )
