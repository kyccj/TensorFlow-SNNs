"""vmem silent_only — 약한 lambda 쪽 곡선 스윕 (26-08-30 등록)

지금 이 방법의 점들 (전부 lambda >= 3e-7, 즉 센 쪽):
...
**점 하나를 반복하는 대신 약한 쪽으로 곡선을 그린다.** 곡선이 일관되게 기준선 위면
한 점의 n 을 늘리는 것보다 강한 근거가 된다.

곡선 지수 0.481 (46,749 @ 3e-7, 36,567 @ 5e-7) 로 외삽:
    lambda=1.5e-7 -> ~65,200    1.8e-7 -> ~59,800
    lambda=2.3e-7 -> ~53,100    2.6e-7 -> ~50,100

**두 점은 기존 1-s 점과 정확히 맞물린다** (직접 비교가 된다):
    ~59,800  <->  1-s lambda=5e-8   95.03% @ 60,215  (n=1)
    ~53,100  <->  1-s lambda=1e-7   94.89% @ 53,160  (n=3)  <- 현재 무손실 최심점

판정 — 이 두 자리에서 vmem 이 1-s 보다 뚜렷이 높으면 방법이 나은 것이다.
...
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_vmweak')
...
#        이름        gain    lambda
JOBS = [
    ('g100_2.3e-7', '1.0', '2.3E-7'),   # ~53,100  <- 1-s λ=1e-7 (94.89%, n=3) 과 같은 자리
    ('g100_1.8e-7', '1.0', '1.8E-7'),   # ~59,800  <- 1-s λ=5e-8 (95.03%) 과 같은 자리
    ('g100_2.6e-7', '1.0', '2.6E-7'),   # ~50,100
    ('g100_1.5e-7', '1.0', '1.5E-7'),   # ~65,200
]
...
def make_config(gpu_id, name, gain, lmb):
    ...
    c, n = RE_NAME.subn(f"conf.exp_set_name='vw-{name}'", c)
    if n < 1:
...
def preflight(jobs):
    ...
    for name, gain, lmb in jobs:
        tgt = os.path.join(PROJECT_ROOT, f'vw-{name}')
...
def main():
    ...
    preflight(pending)
    print(f'--- vmem silent_only 약한 λ 곡선: {len(pending)} runs ---', flush=True)
    while pending:
