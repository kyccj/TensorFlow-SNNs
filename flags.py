...
flags.DEFINE_bool('reg_spike_maxnorm_no_inv',False,"1-maxnorm 가지(reg_spike_out_sc_maxnorm)에서 **1- 반전만** 끈다: sc_rate = sc/max (많이 쏜 뉴런이 세게 눌리는 maxnorm 방향). 기본 False 가 지금까지의 모든 런과 비트 단위로 같다. 기존 reg_spike_out_sc_maxnorm_plain 과의 차이가 이 플래그의 존재 이유다: plain 은 별도 가지라 reg_spike_maxnorm_group 과 reg_spike_vmem_gain 을 아예 안 읽어서 반전·within_channel·vmem 세 개가 동시에 빠진다 -- 어블레이션 표에 '반전의 효과'로 쓸 수 없다. 이 플래그는 group·vmem 처리를 마친 sc_norm 을 그대로 써서 반전 하나만 제거한다. 침묵 뉴런은 vmem_gain=0 이면 sc_rate=0 이 돼 wta_rev backward 가 기울기를 안 주고(의도된 동작), vmem_gain>0 이면 readiness 만큼 [0,1) 로 퍼져 정확히 0 은 아니다. reg_spike_shape_beta 는 무시된다 (1- 가 없으면 단순 배율이라 loss-ratio 가 lambda 로 흡수한다). reg_spike_out_sc_maxnorm 이 켜져 있어야 의미가 있다.")
...
flags.DEFINE_bool('reg_spike_wta_rev_w1',False,"wta_rev 의 spike 기울기를 sc_rate 한 번(w^1)으로 만든다. 기본(False)은 지금까지 돌린 모든 런의 동작이고, 거기서 실효 기울기는 설계 의도인 w 가 아니라 w^2 다: neurons.py 가 x = spike*sc_rate 를 custom_gradient 바깥에서 곱해 넘기므로, l2_norm_wta_rev 가 돌려주는 dy/dx = sc_rate/||x|| 에 autodiff 가 dx/dspike = sc_rate 를 한 번 더 곱한다 (sc_rate 는 tf.Variable 인 spike_count 에서 나와 기울기가 끊겨 있으므로 이 경로 하나뿐). 결과: dL/dspike = lambda*sc_rate^2/||x||. True 로 두면 곱셈을 l2_norm_wta_rev_w1 안으로 옮겨 반환값이 곧 spike 에 대한 기울기가 되고 dL/dspike = lambda*sc_rate/||x|| 가 된다. forward(sqrt(sum((spike*sc_rate)^2)))는 두 경로가 완전히 같으므로 R 과 loss-ratio 는 비교 가능하다. 1-maxnorm 처럼 sc_rate in [0,1] 이면 w^2 는 침묵 뉴런(sc_rate=1) 대비 중간 활동 뉴런의 압력을 제곱으로 깎으므로 가중이 더 뾰족하다. reg_spike_out_wta_rev 가 켜져 있어야 의미가 있고, reg_spike_accum_loss / reg_spike_out_inv_s 와는 같이 쓸 수 없다 (neurons.py 에서 assert).")
