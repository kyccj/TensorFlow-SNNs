...
나머지는 스펙 §8 이 제안한 배치를 기본값으로 하되, 8쪽 여유를 봐서 본문에 올리고 안 되면
부록으로 내린다. **T2 는 2026-09-14 fix round 1 로 상태가 바뀌었다 — 게이트에 걸린 게
아니라 단순히 아직 만들어지지 않았다(미작성).** 문헌 값을 수기로 모아야 하는데 그 수집이
아직 안 됐다. 본문 최소 구성에는 원래도 없었지만, "8쪽 여유가 있으면 올린다" 던 후보군
에서도 뺀다 — 수집 전에는 올릴 표 자체가 없다.
...
| T2 | 선행 연구 대비 (SOTA 표) | **미작성 — 문헌 값 수집 필요** (§2.2/§2.3 인용 논문의 보고 수치를 아직 수기로 모으지 않음) | 게이트 아님, 단순 미작성 | **미작성** (본문 후보에서도 제외, 2026-09-14 fix round 1) | 표가 작고(수기 입력, 성능 축만) 독자가 §1 헤드라인을 문헌 맥락에 놓는 데 도움이 되겠지만, 값을 모으기 전에는 만들 수 없다. `paper/tables/`에 `t2_*.tex` 파일 자체가 없고 `.tex` 어디에서도 참조되지 않음(Task 15 검증) |
...
- 09-14 세트로 F2·F3·F4 를 다시 생성해 확인함 (아래 Step 5 검증 로그의 실행 결과 참조):
  회귀 계수 0.8692(SE 0.0065, t=-21.5, n=21), 최근접 쌍 prop 193,129 / l2 193,083.
  **2026-09-14 fix round 1 갱신** — 이 숫자는 사전 등록 필터(규칙 1·2, 아래 절 참조)를
  적용한 뒤의 것이다. 이전 로그(회귀 계수 0.8673/0.8629/0.8626 등, n=23)는 `neuron_usage.py`
  가 완주·S30/S1 검사 없이 STORE 의 모든 런을 받아들여서 나온, 재현 불가능한 값이었다 —
  `prop-3e-3-s6/s7`(둘 다 26에폭 미완주)이 STORE 상황에 따라 포함되거나 빠지면서
  결과가 흔들렸다. 필터 적용 후 값(0.869, t=-21.5, n=21)은 spec §2 A2·본문 프로즈가
  이미 쓰고 있던 수치와 정확히 일치한다 — **이 숫자가 이제 정본이자 재현 가능한 값이다.**

## 사전 등록 필터 (2026-09-14 fix round 1)

`neuron_usage.py` 는 이제 `collect_paper.py` 의 판정 로직을 재사용해 규칙 1(310 에폭
완주)·2(S30/S1 ≥ 0.19) 를 통과하지 못한 런을 회귀·F2/F3/F4 계산에서 뺀다. **규칙
3(조건당 상위 4개)은 적용하지 않는다** — 보고용 표의 n 을 표준화하는 규칙이지 회귀에서
유효한 점을 버릴 이유가 아니기 때문이다. 배제되는 런은 스크립트가 실행 시 이름과 사유를
그대로 찍는다 (아래 실행 로그의 `r19c10-prop-3e-3-s6`/`s7`, 둘 다 26에폭 미완주).

## 스파이크 축 프로토콜 (2026-09-14 fix round 1)

`neuron_usage.py` 의 스파이크 수(S)는 `reg_detail.csv` 의 **학습 모드** spike_count 합이다.
`dead_neuron_ratio`(활성 뉴런 A 계산에 필요)가 이 CSV 세트에만 있어서 A 와 같은 측정
경로를 쓰려면 이 프로토콜을 벗어날 수 없다. T1 등 논문 표의 스파이크 수는 **eval/test
모드** `s_count`(`train.log`, spec §9-G)이고 값이 다르다 — 최근접 쌍의 두 런을 직접
대조하면 제안법 런은 학습 모드 193,129 vs eval 모드 188,208 (+2.6%), plain L2 런은
학습 모드 193,083 vs eval 모드 187,911 (+2.7%)이다. §4.5 본문과 F2/F3/F4 캡션 모두
이 프로토콜 차이와 실측 오프셋을 명시한다 — "193K" 를 T1 의 "≈192K" 와 같은 것으로
읽지 않는다.
...
## 그림 스크립트 실행 확인 (2026-09-14, fix round 1 로 재실행)

```
$ cd EIP_fig && /home/kyccj/anaconda3/envs/venv_1/bin/python neuron_usage.py
{'base': 5, 'l2': 10, 'prop': 11, 'sm': 3}
사전 등록 필터(규칙 1·2)로 배제된 런 2개:
  r19c10-prop-3e-3-s6: 미완주(26~27ep)
  r19c10-prop-3e-3-s7: 미완주(26~27ep)
저장: .../EIP_fig/neuron_usage_26-09-14.png
회귀: 비율 0.8692  SE 0.0065  t -21.5  n=21
...
두 스크립트 모두 오류 없이 실행되어 PNG 를 갱신했다. (`prop-3e-3-s6/s7` 은 아직 진행
중인 공유 스토리지 런으로 보인다 — 재실행마다 에폭 수가 26↔27 로 흔들린다. 310 에폭에
도달하기 전에는 어느 값이든 규칙 1 로 배제되므로 회귀·n 에는 영향이 없다.)
