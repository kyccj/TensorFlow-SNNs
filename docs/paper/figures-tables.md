...
| T9 | 활성 뉴런 축 전체 표 (09-13, 팔별 스파이크/활성뉴런/스파이크당뉴런) | 수기 집계 (스펙 §8 "층별 세부값" 아래 원자료 표를 그대로 전재) | 없음 — **아직 후보, 정식 채택 안 됨** | 부록 | 스펙 §8 이 "표 T9 후보 — 부록" 으로만 표기하고 본문 배치를 지시하지 않음. F4(같은 예산에서 쓰는 뉴런 수)가 이미 본문에서 같은 축(스파이크당 뉴런)을 다루므로, 이 표는 팔별 전체 수치를 원할 때 참고하는 부록 보충 자료로만 둔다 |
...
| F6 | 에폭별 λ 궤적과 정확도 (ep200 cutmix 종료 반응) | `cd EIP_fig && /home/kyccj/anaconda3/envs/venv_1/bin/python lambda_epoch.py` | **§9-B — ep201 급락이 LR 스케줄과 겹치는지 미확인** | 부록 | 스펙 §9-B: "F6 과 CONTRIB-2 를 쓰기 전에 스케줄 겹침을 확인해야 한다." 미확인 상태로 본문에 실으면 "스케줄과 무관하다"는 주장을 암묵적으로 하게 되므로, 확인 전까지는 부록에 두고 본문에서는 T1/T3 수치로만 CONTRIB-2 를 뒷받침한다 |
