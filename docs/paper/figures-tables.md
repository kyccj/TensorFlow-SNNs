...
| ID | 내용 | 생성 명령 | 게이트 | 배치 | 근거 |
|---|---|---|---|---|---|
| F1 | 정확도–스파이크 곡선, 가중 설계 13종(+baseline)이 한 곡선에 — 09-14 수치는 19종이었으나 Task 13 재검증 결과 실제 유효 런이 남은 설계는 13종 (아래 "F1 데이터·대표점" 절 참조) | `EIP_fig/pareto_curve.py` | 없음 | **본문 (필수)** | 스펙이 본문으로 못 박음. A8(설계 등가성)의 유일한 시각적 증거이자 P4 의 핵심 관찰 |
...
**이 문서와 `paper/sections/experiments.tex` 은 "13종(+baseline, 14점)"으로 통일한다
— "19종"을 그대로 쓰지 않는다.** 개요·스펙의 "19종" 문구는 이 재검증 이전의 수치이며,
이 발견을 개요·스펙에 소급 반영할지는 Task 13 범위 밖이라 team-lead 보고에 남긴다.
