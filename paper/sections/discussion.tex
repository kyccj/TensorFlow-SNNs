...
During the middle portion of training, the ratio controller reduces spikes more effectively than
a fixed-$\lambda$ comparison without sacrificing accuracy. We do not generalise this beyond the observed window, nor do we quantify the reduction without gate G2,
% gate ref: spec \S 2 B0
since the fixed-$\lambda$ arm currently has only two runs at different settings,
too few to separate the controller's trajectory over training from its time-averaged strength.
The detailed conditions and analysis for this result are in the appendix.
...
disagree by a measurable margin
(\S\ref{sec:exp-analysis}, where training-mode spike counts are reported) that leaving the choice implicit would risk comparing
