...
SWEEP_DIR = os.path.join(PROJECT_ROOT, '_aclr')
...
#        이름          accum   rho
JOBS = [
    ('on_1.2e-3',  'True',  '0.0012'),
    ('off_1.2e-3', 'False', '0.0012'),
    ('on_3e-3',    'True',  '0.003'),
    ('off_3e-3',   'False', '0.003'),
]
...
def make_config(gpu_id, name, gain, lmb):
    ...
    c, n = RE_NAME.subn(f"conf.exp_set_name='lr-{name}'", c)
    if n < 1:
...
def preflight(jobs):
    ...
    for name, gain, lmb in jobs:
        tgt = os.path.join(PROJECT_ROOT, f'lr-{name}')
